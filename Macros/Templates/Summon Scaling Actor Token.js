if (args[0] === "on") {
	...
	let summonID = await warpgate.spawn(summonType, updates, callbacks, options); // Summon token and create variable with summoned token ID
	actor.update({"flags.summons.CHANGETHISNAMEHERE" : summonID}); //Change the name of flag to be something similar to the summonType, just without spaces
} else if (args[0] === "off") {
	await warpgate.dismiss(actor.data.flags.summons.CHANGETHISNAMEHERE); // Still have to figure this one out
};
